...
Этот процесс необходим для повышения эффективности и качества работы агента. Он замыкает цикл разработки, превращая проблемы в обновленные инструкции/правила для агента и конфигурации утилит для проверки кода.

1. **Наблюдение.** Наблюдаем за работой агента. На всех этапах его работы фиксируем проблемы, с которыми он сталкивается, а также проблемы, которые выявляются на код-ревью.
2. **Анализ.** Выявляем наиболее важные проблемы, которые приводят к замедлению работы агента, к трате лишних токенов, к трате моего времени, и придумываем решение.
3. **Внесение улучшений.** На основе анализа дорабатываем процессы, инструкции, промты и прочее, что посчитали нужным.

```mermaid
flowchart LR
    A[Наблюдение] --> B[Анализ]
    B --> C[Внесение улучшений]
    C --> A
```

> **Важно:** не нужно за раз вносить массу изменений, так как мы не сможем понять, что и как влияет на работу агента. Нужно внести несколько изменений, понаблюдать и перейти к шагу 1.
