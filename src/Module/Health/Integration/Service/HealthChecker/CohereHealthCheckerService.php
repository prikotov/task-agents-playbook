...
use Common\Module\Llm\Application\UseCase\Query\CheckCohereHealth\CheckCohereHealthQuery;
use Common\Module\Llm\Application\UseCase\Query\CheckCohereHealth\CohereHealthDto;
use Override;

/**
 * Integration Service для проверки здоровья Cohere API.
 *
 ...
 */
final readonly class CohereHealthCheckerService implements CheckHealthServiceInterface
{
    private const string SERVICE_NAME = 'cohere';
    ...
    public function check(): HealthCheckResultVo
    {
        ...
        /** @var CohereHealthDto $result */
        $result = $this->queryBus->query(new CheckCohereHealthQuery());

        $responseTimeMs = round((microtime(true) - $startTime) * 1000.0, 2);

        if ($result->isHealthy) {
            return HealthCheckResultVo::createFromOperational(
                message: sprintf('Cohere API is operational (%d models available)', $result->modelsCount),
                responseTimeMs: $responseTimeMs,
            );
        }

        return HealthCheckResultVo::createFromOutage(
            message: sprintf('Cohere API health check failed: %s', $result->errorMessage ?? 'unknown error'),
            responseTimeMs: $responseTimeMs,
        );
    }
}
