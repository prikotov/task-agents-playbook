...
/**
 * Integration Service для проверки здоровья yt-dlp.
 *
 ...
 */
final readonly class YtDlpHealthCheckerService implements CheckHealthServiceInterface
{
    private const string SERVICE_NAME = 'yt-dlp';
    ...
    public function check(): HealthCheckResultVo
    {
        ...
        if ($result->isHealthy) {
            return HealthCheckResultVo::createFromOperational(
                message: sprintf('yt-dlp is operational (version: %s)', $result->version ?? 'unknown'),
                responseTimeMs: $responseTimeMs,
            );
        }

        return HealthCheckResultVo::createFromOutage(
            message: sprintf('yt-dlp health check failed: %s', $result->errorMessage ?? 'unknown error'),
            responseTimeMs: $responseTimeMs,
        );
    }
}
