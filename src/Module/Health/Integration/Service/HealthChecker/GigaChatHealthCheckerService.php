...
use Common\Module\Llm\Application\UseCase\Query\CheckGigaChatHealth\CheckGigaChatHealthQuery;
use Common\Module\Llm\Application\UseCase\Query\CheckGigaChatHealth\GigaChatHealthDto;
use Override;

/**
 * Integration Service для проверки здоровья GigaChat API.
 *
 ...
 */
final readonly class GigaChatHealthCheckerService implements CheckHealthServiceInterface
{
    private const string SERVICE_NAME = 'gigachat';
    ...
    public function check(): HealthCheckResultVo
    {
        ...
        /** @var GigaChatHealthDto $result */
        $result = $this->queryBus->query(new CheckGigaChatHealthQuery());

        $responseTimeMs = round((microtime(true) - $startTime) * 1000.0, 2);

        if ($result->isHealthy) {
            return HealthCheckResultVo::createFromOperational(
                message: 'GigaChat API is operational',
                responseTimeMs: $responseTimeMs,
            );
        }

        return HealthCheckResultVo::createFromOutage(
            message: sprintf('GigaChat API health check failed: %s', $result->errorMessage ?? 'unknown error'),
            responseTimeMs: $responseTimeMs,
        );
    }
}
